/****************************************************************************
 * boards/infineon/boards/aurix/tc397_evb/scripts/Lcf_Tasking_Tricore_Tc.lsl
 *
    ...
    section_layout :vtc:linear
    {
        ...
        group
        {
            ...
            group
            {
                ...
                {
                    select "(.lmudata_cpu1|.lmudata_cpu1.*)";
                    select "(.lmubss_cpu1|.lmubss_cpu1.*)";
                }
                group (ordered, attributes=rw, run_addr = mem:cpu2_dlmu)
                {
                    select "(.lmudata_cpu2|.lmudata_cpu2.*)";
                    select "(.lmubss_cpu2|.lmubss_cpu2.*)";
                }
                group (ordered, attributes=rw, run_addr = mem:cpu3_dlmu)
                {
                    select "(.lmudata_cpu3|.lmudata_cpu3.*)";
                    select "(.lmubss_cpu3|.lmubss_cpu3.*)";
                }
                group (ordered, attributes=rw, run_addr = mem:cpu4_dlmu)
                {
                    select "(.lmudata_cpu4|.lmudata_cpu4.*)";
                    select "(.lmubss_cpu4|.lmubss_cpu4.*)";
                }
                group (ordered, attributes=rw, run_addr = mem:cpu5_dlmu)
                {
                    select "(.lmudata_cpu5|.lmudata_cpu5.*)";
                    select "(.lmubss_cpu5|.lmubss_cpu5.*)";
                }
                group (ordered, attributes=rw, run_addr=mem:lmuram)
                {
                    select "(.data.lmudata|.data.lmudata.*)";
                    select "(.lmubss|.lmubss.*)";
                }
            }
        }
