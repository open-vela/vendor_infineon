  ...
  if (!priv->spichannel[devid].status)
    {
      ...
      IfxQspi_initSlso(slso->pin, slso->mode, slso->driver,
                       slso->pin->select);
                                        ...
                                        uint32_t frequency)
{
   ...
   * time_clock = 1000000000 / (float)IfxClock_getQspiFrequency(); = 5ns
                               ...
                               uint32_t csdelay, uint32_t ifdelay)
{
  ...
  freq = IfxClock_getQspiFrequency();
  if (freq == 0)
    {
      return -EIO;
    }

  time_clock = 1000000000 / freq;
...
static int aurix_qspi_receive(int irq, void *context, void *arg)
{
  ...
  if (priv->qspi.activeChannel->flags.onTransfer == 0)
    {
      nxsem_post(&priv->sem);
    }
  ...
  for (i = 0; i < count; i++)
    {
      ...
      qspi_config.mode        = SpiIf_Mode_master;
      qspi_config.pins        = &config[i].pins;
      qspi_config.isrProvider = (IfxSrc_Tos)IfxCpu_getCoreId();
      qspi_config.txPriority  = IRQ_TO_NDX(config[i].tx_irq);
      qspi_config.rxPriority  = IRQ_TO_NDX(config[i].rx_irq);
      qspi_config.erPriority  = IRQ_TO_NDX(config[i].err_irq);
      qspi_config.vmId        = IfxSrc_VmId_0;
      ...
      for (j = 0; j < config[i].cs_num; j++)
        {
          ...
          channel_config.ch.baudrate = 1000000;
          channel_config.ch.mode.dataWidth = config[i].datawidth;
          channel_config.ch.mode.autoCS = true;
