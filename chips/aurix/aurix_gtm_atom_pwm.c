...
static int aurix_atom_pwm_setup(struct pwm_lowerhalf_s *dev)
{
  struct aurix_gtm_atom_pwm_dev_s *priv =
    (struct aurix_gtm_atom_pwm_dev_s *)dev;
...
static int aurix_atom_pwm_shutdown(struct pwm_lowerhalf_s *dev)
{
  ...
  IfxGtm_Atom_Pwm_stop(&priv->atom_driver, TRUE);
  ...
  const struct pwm_info_s *info)
{
  ...
  IfxGtm_Cmu_Clk fxclk_src = (IfxGtm_Cmu_Clk)priv->atom_ch_clk;

  float32 ch_src_freq =
  IfxGtm_Cmu_getClkFrequency(&MODULE_GTM, fxclk_src, FALSE);
  ...
  if (priv->running == false)
    {
      ...
      IfxGtm_Atom_Pwm_init(&priv->atom_driver, &priv->atom_config);
      priv->running = true;
    }
  else
    {
      Ifx_GTM_ATOM *atomSFR =
          &priv->atom_config.gtm->ATOM[priv->atom_config.atom];

      IfxGtm_Atom_Ch_setCompareZeroShadow(atomSFR,
          priv->atom_config.atomChannel, priv->atom_config.period);
      IfxGtm_Atom_Ch_setCompareOneShadow(atomSFR,
 ...
 *
 * Input Parameters:
 *   cfgs  - Configuration for the GTM ATOM channel.
      ...
      if (cfgs[i].module_type == GTM_Module_Type_ATOM)
        {
          ...
          devs[i] = (struct pwm_lowerhalf_s *)
              kmm_zalloc(sizeof(struct aurix_gtm_atom_pwm_dev_s));
          if (devs[i] == NULL)
            {
              return -ENOMEM;
            }

          priv = (struct aurix_gtm_atom_pwm_dev_s *)devs[i];

          /* Initialize the pwm channel device. */

          IfxGtm_Atom_Pwm_initConfig(&priv->atom_config, &MODULE_GTM);
          priv->atom_config.atom = (IfxGtm_Atom)cfgs[i].module_id;
          priv->atom_config.atomChannel = (IfxGtm_Atom_Ch)cfgs[i].channel_id;
          priv->atom_ch_clk = (IfxGtm_Cmu_Clk)cfgs[i].ch_clk_id;
