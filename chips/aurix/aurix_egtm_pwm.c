...
static int aurix_pwm_setup(struct pwm_lowerhalf_s *dev)
{
  struct aurix_egtm_tom_pwm_dev_s *priv =
    (struct aurix_egtm_tom_pwm_dev_s *)dev;
...
static int aurix_pwm_shutdown(struct pwm_lowerhalf_s *dev)
{
  ...
  IfxEgtm_Tom_Pwm_stop(&priv->tom_driver, TRUE);
  ...
  const struct pwm_info_s *info)
{
  ...
  IfxEgtm_Cmu_Fxclk fxclk_src = (IfxEgtm_Cmu_Fxclk)priv->tom_config.clock;

  float32 ch_src_freq = IfxEgtm_Cmu_getFxClkFrequency
    (&MODULE_EGTM, fxclk_src, FALSE);
  ...
  if (priv->running == false)
    {
      /* Initialize the EGTM TOM */

      IfxEgtm_Tom_Pwm_init(&priv->tom_driver, &priv->tom_config);
      priv->running = true;
    }
  else
    {
      Ifx_EGTM_CLS_TOM *tomSFR =
          &priv->tom_config.egtm->CLS[priv->tom_config.cluster].TOM;

      IfxEgtm_Tom_Ch_setCompareZeroShadow(tomSFR,
        priv->tom_config.tomChannel,
        priv->tom_config.period);
      IfxEgtm_Tom_Ch_setCompareOneShadow(tomSFR,
        priv->tom_config.tomChannel,
      ...
      if (cfgs[i].module_type == GTM_Module_Type_TOM)
        {
          ...
          devs[i] = (struct pwm_lowerhalf_s *)
            kmm_zalloc(sizeof(struct aurix_egtm_tom_pwm_dev_s));
          if (devs[i] == NULL)
            {
          ...
          priv = (struct aurix_egtm_tom_pwm_dev_s *)devs[i];

          /* Initialize the pwm channel device. */

          IfxEgtm_Tom_Pwm_initConfig(&priv->tom_config, &MODULE_EGTM);
          priv->tom_config.cluster = (IfxEgtm_Cluster)cfgs[i].module_id;
          priv->tom_config.tomChannel = (IfxEgtm_Tom_Ch)cfgs[i].channel_id;
          priv->tom_config.clock = (IfxEgtm_Tom_Ch_ClkSrc)cfgs[i].ch_clk_id;
          ...
          /* Config TOM channel as ADC trigger and start. */

          switch (cfgs[i].adc_trig)
            {
              case TOM_ADC_TRIG_NONE:
                break;

              case TOM_ADC_TRIG_SEL0:
                MODULE_EGTM.ADC.OUT[cls].B.SEL0 = EGTM_ADC_TRIG_TOMx_0 + ch;
                break;

              case TOM_ADC_TRIG_SEL1:
                MODULE_EGTM.ADC.OUT[cls].B.SEL1 = EGTM_ADC_TRIG_TOMx_0 + ch;
                break;

              case TOM_ADC_TRIG_SEL2:
                MODULE_EGTM.ADC.OUT[cls].B.SEL2 = EGTM_ADC_TRIG_TOMx_0 + ch;
                break;

              case TOM_ADC_TRIG_SEL3:
                MODULE_EGTM.ADC.OUT[cls].B.SEL3 = EGTM_ADC_TRIG_TOMx_0 + ch;
                break;
            }

          if (cfgs[i].adc_trig != TOM_ADC_TRIG_NONE)
            {
              info.frequency = (uint32_t)cfgs[i].adc_trig_freq;
              info.duty = 32768u;
              aurix_pwm_start(devs[i], &info);
            }
        }
    }
