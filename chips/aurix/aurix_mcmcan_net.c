...
static void aurix_mcmcan_node_allint(struct aurix_mcmcan_priv_s *priv,
                                     boolean enable);
...
static_assert(CONFIG_IOB_BUFSIZE >= sizeof(struct canfd_frame) + CONFIG_NET_LL_GUARDSIZE,
              "iob size too small");
...
/****************************************************************************
 * Name: aurix_mcmcan_node_allint
 *
 * Description:
 *   Enable or disable all CAN node interrupts (TX/RX/error) together,
 *   including both the peripheral interrupt sources and their CPU lines.
 *
 * Input Parameters:
 *   priv   - An instance of the CAN driver structure.
 *   enable - True to enable all interrupts, false to disable and clear them.
 *
 * Returned Value:
 *   None
 *
                                ...
                                enum can_ioctl_state_e state)
{
  ...
  if (!IFF_IS_UP(priv->dev.netdev.d_flags))
    {
      return -ENETDOWN;
    }
  ...
  switch (state)
    {
      case CAN_STATE_OPERATIONAL:
        ...
        aurix_mcmcan_node_allint(priv, true);
      ...
      case CAN_STATE_STOPPED:
        ...
        if (priv->state == CAN_STATE_SLEEP)
        {
          nerr("ERROR: can't set to stopped state from sleep state!\n");
          return -ENOTTY;
        }

        aurix_mcmcan_node_allint(priv, false);
...
static void aurix_mcmcan_co_shutdown(struct aurix_mcmcan_priv_s *priv)
{
  ...
  /* cancel errpolling cycle thread */

#ifdef CONFIG_AURIX_CAN_ERROR_POLLING
  work_cancel(LPWORK, &priv->errwork);
#endif
  ...
  if (shared_data_manual.can_module[ref_index].can_module_ref == 0 &&
      IfxCan_isModuleEnabled(priv->node_group->can) == TRUE)
    {
      ...
      IfxCan_resetModule(priv->node_group->can);
      mcmcan_disable_mcan_module(priv->node_group->can);
    }
...
static int aurix_mcmcan_rx_interrupt(int irq, void *context, void *arg)
{
  ...
  if (IfxCan_Node_getInterruptFlagStatus(can_node->node,
    ...
    {
      priv->rxfifo0_pending = IfxCan_Node_getRxFifo0FillLevel(can_node->node);
    }

  if (IfxCan_Node_getInterruptFlagStatus(can_node->node,
      IfxCan_Interrupt_rxFifo1NewMessage))
    {
      priv->rxfifo1_pending = IfxCan_Node_getRxFifo1FillLevel(can_node->node);
    }
#endif
                                       ...
                                       uint8 buffer_type, uint8 buffer_id)
{
  ...
  frame->flags |= rxbuffer_element->R1A.B.FDF ? CANFD_FDF : 0;
  frame->flags |= rxbuffer_element->R1A.B.BRS ? CANFD_BRS : 0;
                             ...
                             size_t num)
{
  ...
  if (IfxClock_getMcanFrequency() != CONFIG_AURIX_MCMCAN_CLOCK)
    {
      IfxClock_setMcanFrequency(CONFIG_AURIX_MCMCAN_CLOCK);
    }

  if (IfxClock_getMcanhFrequency() != CONFIG_AURIX_MCMCAN_ACCESS_RAM_CLOCK)
    {
      IfxClock_setMcanhFrequency(CONFIG_AURIX_MCMCAN_ACCESS_RAM_CLOCK);
    }
