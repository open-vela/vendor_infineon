...
struct aurix_egtm_tom_timer_lowerhalf_s
{
...
static int aurix_egtm_tom_start(struct timer_lowerhalf_s *lower);
static int aurix_egtm_tom_stop(struct timer_lowerhalf_s *lower);
static int aurix_egtm_tom_getstatus(struct timer_lowerhalf_s *lower,
                                     struct timer_status_s *status);
static int aurix_egtm_tom_settimeout(struct timer_lowerhalf_s *lower,
                                      uint32_t timeout);
static void aurix_egtm_tom_setcallback(struct timer_lowerhalf_s *lower,
                                        tccb_t callback, void *arg);
static int aurix_egtm_tom_maxtimeout(struct timer_lowerhalf_s *lower,
                                      uint32_t *maxtimeout);

static int aurix_egtm_tom_handler(int irq, void *context, void *arg);
...
{
  .start       = aurix_egtm_tom_start,
  .stop        = aurix_egtm_tom_stop,
  .getstatus   = aurix_egtm_tom_getstatus,
  .settimeout  = aurix_egtm_tom_settimeout,
  .setcallback = aurix_egtm_tom_setcallback,
  .ioctl       = NULL,
  .maxtimeout  = aurix_egtm_tom_maxtimeout,
};
...
/****************************************************************************
 * Name: aurix_egtm_tom_handler
 *
 * Description:
 *   one channel timer interrupt handler of egtm-tom module
 *
 ****************************************************************************/

static int aurix_egtm_tom_handler(int irq, void *context, void *lower)
{
  struct aurix_egtm_tom_timer_lowerhalf_s *priv =
    (struct aurix_egtm_tom_timer_lowerhalf_s *)lower;
  uint32_t next_interval_us = 0;
  bool notify = false;

  notify = IfxEgtm_Tom_Ch_isZeroNotification(priv->timer.tom,
                                              priv->timer.timerChannel);
  if (notify)
    {
      IfxEgtm_Tom_Ch_clearZeroNotification(priv->timer.tom,
                                            priv->timer.timerChannel);
      if (priv->callback)
        {
          if (priv->callback(&next_interval_us, priv->arg))
            ...
            {
              aurix_egtm_tom_stop((struct timer_lowerhalf_s *)lower);
            }
        }
    }
...
static int aurix_egtm_tom_start(struct timer_lowerhalf_s *lower)
{
  ...
  IfxEgtm_Tom_Ch_setCounterValue(priv->timer.tom,
...
static int aurix_egtm_tom_stop(struct timer_lowerhalf_s *lower)
{
  ...
  IfxEgtm_Tom_Timer_stop(&priv->timer);
  IfxEgtm_Tom_Ch_clearZeroNotification(priv->timer.tom,
                                        priv->timer.timerChannel);
...
/****************************************************************************
 * Name: aurix_egtm_tom_setcallback
 *
 * Description:
...
/****************************************************************************
 * Name: aurix_egtm_tom_settimeout
 *
 * Description:
                                      ...
                                      uint32_t timeout)
{
  ...
  IfxEgtm_Tom_Ch_setCompareZero(priv->timer.tom,
                                 priv->timer.timerChannel,
                                 maxtimeout);
  freq = IfxEgtm_Tom_Ch_getClockFrequency(priv->timer.egtm,
                                           priv->timer.tom,
                                           priv->timer.timerChannel);
...
/****************************************************************************
 * Name: aurix_egtm_tom_maxtimeout
 *
 * Description:
                                     ...
                                     struct timer_status_s *status)
{
  ...
  timeouttick = IfxEgtm_Tom_Ch_getCompareZero(priv->timer.tom,
                                               priv->timer.timerChannel);
  ...
  currenttick = *IfxEgtm_Tom_Ch_getTimerPointer(priv->timer.tom,
                                                 priv->timer.timerChannel);
 ...
 *   cfgs - The list of configuration for the eGTM-TOM channel.
  ...
  for (i = 0; i < num && dev[i] != DEV_END; i++)
    {
      ...
      priv = kmm_zalloc(sizeof(struct aurix_egtm_tom_timer_lowerhalf_s));
      if (priv == NULL)
        {
      ...
      memset(priv, 0, sizeof(struct aurix_egtm_tom_timer_lowerhalf_s));
      ...
      IfxEgtm_Tom_Timer_init(&priv->timer, &timerConfig);

      if (config[i].oneshotmode == TRUE)
        {
          IfxEgtm_Tom_Ch_setOneShotMode(priv->timer.tom,
